      ...
      >
        ...
        {children || (
          <div className="flex flex-col items-center text-center">
            <div className={cn('rounded-full bg-slate-100 p-3 mb-3 transition-colors', isDragActive && 'bg-emerald-100')}>
              <UploadIcon className={cn('h-6 w-6 transition-colors', isDragActive ? 'text-emerald-600' : 'text-slate-400')} />
            </div>
            {isDragActive ? (
          ...
          {src.map((file, i) => (
            <div key={i} className="relative aspect-square rounded-lg overflow-hidden border border-slate-200 bg-slate-100 group">
              {file.type?.startsWith('image/') ? (
                ...
                <div className="h-full w-full flex items-center justify-center text-slate-400 text-xs font-medium p-1 text-center break-all">
                  {file.name}
                </div>
              )}
              {onRemove && (
