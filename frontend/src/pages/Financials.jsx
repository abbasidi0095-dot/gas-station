    ...
    <div className="space-y-6">
      ...
      <div className="flex border-b border-slate-200">
        <button
          ...
          className={`flex items-center space-x-2.5 px-6 py-3 font-bold text-sm tracking-tight border-b-2 transition-all ${activeTab === 'charges' ? 'border-indigo-600 text-indigo-600 bg-indigo-50/20' : 'border-transparent text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:text-slate-200'}`}
        >
        ...
        <button
          ...
          className={`flex items-center space-x-2.5 px-6 py-3 font-bold text-sm tracking-tight border-b-2 transition-all ${activeTab === 'revenue' ? 'border-indigo-600 text-indigo-600 bg-indigo-50/20' : 'border-transparent text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:text-slate-200'}`}
        >
      ...
      <div className="bg-slate-900 text-white rounded-2xl p-5 shadow-inner border border-slate-800 flex justify-between items-center">
        <div>
          <span className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-wider block">{t('ledgerTotal')}</span>
          <span className="text-sm text-slate-400 dark:text-slate-500 mt-1">{t('ledgerTotalDesc')}</span>
        </div>
      ...
      {/* Filters */}
      <div className="bg-white dark:bg-slate-800/80 p-5 rounded-2xl border border-slate-100 dark:border-slate-700/50 shadow-sm dark:shadow-slate-900/50 space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2">
            <Filter className="h-4 w-4 text-slate-400 dark:text-slate-500" />
            <h4 className="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider">{t('searchFilters')}</h4>
          </div>
          <button onClick={handleClearFilters} className="text-xs font-semibold text-slate-400 dark:text-slate-500 hover:text-indigo-600 transition-colors">{t('clearAll')}</button>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-sm font-medium">
          <div>
            <span className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase block mb-1">{t('category')}</span>
            {activeTab === 'charges' ? (
              <select value={categoryFilter} onChange={(e) => setCategoryFilter(e.target.value)} className="w-full border border-slate-200 rounded-lg px-2.5 py-1.5 text-xs text-slate-700 dark:text-slate-300">
          ...
          {activeTab === 'charges' && (
            <div>
              <span className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase block mb-1">{t('fuelVendor')}</span>
              <select value={vendorFilter} onChange={(e) => setVendorFilter(e.target.value)} className="w-full border border-slate-200 rounded-lg px-2.5 py-1.5 text-xs text-slate-700 dark:text-slate-300">
          ...
          <div>
            <span className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase block mb-1">{t('startDate')}</span>
            <input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} className="w-full border border-slate-200 rounded-lg px-2.5 py-1.5 text-xs text-slate-700 dark:text-slate-300" />
          </div>
          <div>
            <span className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase block mb-1">{t('endDate')}</span>
            <input type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} className="w-full border border-slate-200 rounded-lg px-2.5 py-1.5 text-xs text-slate-700 dark:text-slate-300" />
          </div>
        </div>
      </div>
      ...
      {loading ? (
        <div className="w-full py-12 flex justify-center items-center space-x-2">
          ...
          <span className="text-sm font-semibold text-slate-500 dark:text-slate-400">{t('retrieving')}</span>
        </div>
      ) : (
        <div className="bg-white dark:bg-slate-800/80 rounded-2xl border border-slate-100 dark:border-slate-700/50 shadow-sm dark:shadow-slate-900/50 overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-50 dark:bg-slate-700/50 text-slate-500 dark:text-slate-300 uppercase text-[10px] font-bold tracking-wider">
              <tr>
            ...
            <tbody className="divide-y divide-slate-100 font-medium text-slate-700 dark:text-slate-300">
              {activeTab === 'charges' ? (
                charges.length === 0 ? (
                  ...
                  charges.map((item) => (
                    <tr key={item.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-700/30">
                      <td className="px-5 py-3.5 font-bold text-slate-900 dark:text-slate-50">{new Date(item.date).toLocaleDateString()}</td>
                      <td className="px-5 py-3.5 font-semibold text-slate-800 dark:text-slate-200">{item.vendor ? item.vendor.name : <span className="text-slate-400 dark:text-slate-500">—</span>}</td>
                      <td className="px-5 py-3.5">{catLabel(item.category)}</td>
                      <td className="px-5 py-3.5 text-slate-500 dark:text-slate-400 max-w-xs truncate" title={item.description}>{item.description || '—'}</td>
                      <td className="px-5 py-3.5">
                        {item.receiptId ? (
                          <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-bold bg-indigo-50 dark:bg-indigo-900/50 text-indigo-700 dark:text-indigo-300">{t('ocrScan')}</span>
                        ) : (
                ...
                revenue.length === 0 ? (
                  <tr><td colSpan="6" className="px-5 py-8 text-center text-slate-400 dark:text-slate-500">{t('noRevenue')}</td></tr>
                ) : (
                  revenue.map((item) => (
                    <tr key={item.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-700/30">
                      <td className="px-5 py-3.5 font-bold text-slate-900 dark:text-slate-50">{new Date(item.date).toLocaleDateString()}</td>
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-sm">
          <form onSubmit={handleSubmitForm} className="bg-white dark:bg-slate-800/80 rounded-2xl shadow-2xl max-w-md w-full overflow-hidden border">
            <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-700/50 flex justify-between items-center bg-slate-50 dark:bg-slate-800/50">
              <h3 className="font-bold text-slate-900 dark:text-slate-50 text-md">{editingItem ? t('editEntry') : activeTab === 'charges' ? t('recordCharge') : t('logRevenueStream')}</h3>
              <button type="button" onClick={() => setModalOpen(false)} className="p-1 text-slate-400 dark:text-slate-500 hover:text-slate-600"><X className="h-5 w-5" /></button>
            </div>
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider block mb-1">{t('valueAmount')}</label>
                  <input type="number" step="0.01" min="0" required placeholder="0.00" value={amount} onChange={(e) => setAmount(e.target.value)} className="w-full border rounded-lg px-3 py-2 text-sm text-slate-800 dark:text-slate-200" />
                </div>
                <div>
                  <label className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider block mb-1">{t('transactionDate')}</label>
                  <input type="date" required value={date} onChange={(e) => setDate(e.target.value)} className="w-full border rounded-lg px-3 py-2 text-sm text-slate-800 dark:text-slate-200" />
                </div>
              </div>
              <div>
                <label className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider block mb-1">{t('categoryClassification')}</label>
                {activeTab === 'charges' ? (
                  <select value={category} onChange={(e) => setCategory(e.target.value)} required className="w-full border rounded-lg px-3 py-2 text-sm text-slate-800 dark:text-slate-200">
              ...
              {activeTab === 'charges' && category === 'fuel_purchase' && (
                <div>
                  <label className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider block mb-1">{t('supplierMapping')}</label>
                  <select value={formVendorId} onChange={(e) => setFormVendorId(e.target.value)} className="w-full border rounded-lg px-3 py-2 text-sm text-slate-800 dark:text-slate-200">
              ...
              <div>
                ...
                <textarea rows="3" placeholder={t('notesPlaceholder')} value={description} onChange={(e) => setDescription(e.target.value)} className="w-full border rounded-lg px-3 py-2 text-sm text-slate-800 dark:text-slate-200" />
              </div>
            </div>
            <div className="px-6 py-4 bg-slate-50 dark:bg-slate-800/50 border-t border-slate-100 dark:border-slate-700/50 flex space-x-3">
              <button type="button" onClick={() => setModalOpen(false)} className="flex-1 py-2 text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:text-slate-300 text-sm font-semibold">{t('cancel')}</button>
